...
- HEAD de referência desta atualização documental: `3ce5397` (`ajuste wordle`); o SHA final desta documentação deve ser obtido por `git rev-parse HEAD` após integração;
...
- 27.7.5 Correções e Revalidação: `IN_PROGRESS / PROMOTED_AND_WORDLE_REVALIDATED`. Foram consolidados layout e contraste mobile, navegação inferior e botões Voltar, cards compactos, dimensionamento de assets, Perfil, Loja/Inventário, Home, Daily e Administração; foram corrigidos alinhamentos/instruções dos jogos, regra de desbloqueio do Cofre, browser-smoke e registro resiliente/idempotente de resultados. O proprietário confirmou Quality, promoção e revalidação produtiva do Wordle após a correção de conclusão para palavras de 6 e 7 letras.
...
- 27.7.5C.1 Memória dinâmica: `DONE localmente / READY_FOR_27_7_5C_2`. Novas partidas Free, Daily e Event persistem três fontes e compõem um par canônico de cada uma; seleções históricas de fonte única continuam reproduzíveis, sem migration ou mudança de conteúdo.
