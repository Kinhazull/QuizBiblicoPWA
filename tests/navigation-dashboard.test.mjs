...
test("participant navigation has exactly five named destinations and keeps admin contextual", () => {
  ...
  assert.match(chrome, /Abrir painel administrativo|AccountMenu/);
});
...
test("participant controls expose notices settings active state and safe areas", () => {
  ...
  assert.match(chrome, /avisos não lidos/); assert.match(chrome, /aria-label="Configurações"/);
...
test("account menu has explicit secure logout and privileged admin entry", () => {
  const account = read("app/AccountMenu.tsx"), logout = read("app/LogoutButton.tsx");
  for (const label of ["Meu perfil", "Configurações", "Abrir painel administrativo"]) assert.match(account, new RegExp(label));
...
test("admin navigation is one-column collapsible accessible and centrally named", () => {
  ...
  for (const label of ["Aprovações e acessos", "Avisos e comunicados", "Sugestões com IA", "Revisão colaborativa", "Auditoria administrativa"]) assert.match(config, new RegExp(label));
