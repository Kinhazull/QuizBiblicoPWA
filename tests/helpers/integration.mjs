...
export function createTestDatabase(){const raw=new DatabaseSync(":memory:");applyMigrations(raw);const DB=new TestD1(raw);return{raw,DB,env:{DB,MFA_ENCRYPTION_KEY:"MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY"},close:()=>raw.close(),reset:()=>{throw new Error("Create a fresh database per test")}}}
...
export async function createSession(ctx,userId,{token=`token-${userId}`,expiresAt=Date.now()+3600000,mfaVerified=1}={}){ctx.raw.prepare("INSERT INTO sessions(id,user_id,token_hash,persistent,expires_at,last_seen_at,created_at,mfa_verified) VALUES(?,?,?,0,?,?,?,?)").run(`session-${userId}-${token}`,userId,await sha256(token),expiresAt,0,0,mfaVerified);return token}
