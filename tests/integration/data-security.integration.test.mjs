...
test("backup authenticates password and excludes credentials, sessions and tokens",async t=>{
  ...
  const text=await response.text(),data=JSON.parse(text);assert.equal(data.schemaVersion,21);assert.equal(data.credentialsExcluded,true);assert.equal(data.tables.sessions,undefined);
