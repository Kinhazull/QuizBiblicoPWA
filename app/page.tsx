...
export default function Home() {
  ...
  async function submitAuth(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = mfaChallenge
      ...
      : { displayName: data.displayName, username: data.username, password: data.password, inviteCode: data.inviteCode, adultConfirmed: data.adultConfirmed === "on", legalAccepted: data.legalAccepted === "on", termsVersion: LEGAL_VERSION, privacyVersion: LEGAL_VERSION };
    try {
      ...
      if (!response.ok) {
        const messages: any = { invalid_credentials: "Usuário ou senha incorretos.", pending_approval: "Seu cadastro ainda aguarda aprovação.", account_unavailable: "Esta conta não está disponível.", invalid_invitation: "O código de convite é inválido ou expirou.", invitation_limit: "O limite de cadastros deste convite foi atingido.", username_unavailable: "Este nome de usuário já está em uso.", invalid_fields: "Confira os dados. A senha deve ter pelo menos 10 caracteres.", adult_confirmation_required: "Confirme que você tem 18 anos ou mais para criar a conta.", legal_consent_required: "Você precisa aceitar os Termos de Uso e a Política de Privacidade.", too_many_attempts: "Muitas tentativas incorretas. O acesso foi protegido por 15 minutos." };
