...
export class EventContentProvider implements GameContentProvider {
  readonly mode = GameContentMode.EVENT;

  async load<TPayload>(request: GameContentRequest): Promise<LoadedGameContent<TPayload>> {
    if (!request.selectionId || !request.eventId) throw new Error("event_content_provider_invalid_request");
    const start = await requestJson(`/api/platform/events/${encodeURIComponent(request.eventId)}/start`, {
      method: "POST",
      signal: request.signal,
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ selectionId: request.selectionId }),
    ...
    const data = await requestJson(
      `/api/platform/events/${encodeURIComponent(request.eventId)}/selection?selectionId=${encodeURIComponent(request.selectionId)}`,
      { method: "GET", signal: request.signal },
    );
    const game = data.game as Record<string, unknown> | null;
    if (!game || game.selectionId !== request.selectionId || game.gameType !== request.gameType) {
      throw new Error("event_content_selection_mismatch");
    }
    const payload = game.content as Record<string, unknown> | null;
    if (!payload) throw new Error("game_content_invalid");
    const primary = request.gameType === GameType.QUIZ
    ...
    return {
      ...
      metadata: {
        title: typeof game.title === "string" ? game.title : null,
        biblicalReference: typeof primary.biblicalReference === "string" ? primary.biblicalReference : null,
        expiresAt: Number.isFinite(Number(game.expiresAt)) ? Number(game.expiresAt) : null,
      },
    };
  }
}
