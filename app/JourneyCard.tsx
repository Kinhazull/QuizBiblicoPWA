...
export default function JourneyCard({ data, remaining }: { data: JourneyCardData | null; remaining: (target?: number) => string }) {
  ...
  const showOfficialResult = view.tone !== "training" && (Boolean(current) || Number(data?.recent?.bestScore || 0) > 0);
  const positionLabel = current && !completion?.completed ? "Ainda não classificado" : data?.ranking?.provisional ? "Provisório" : "Resultado final";
