...
const icons: Record<string, string> = { waiting: "📅", available: "▶", active: "▶", completed: "🏆", closed: "🏅", training: "🎯" };
...
export default function JourneyCard({ data, remaining }: { data: JourneyCardData | null; remaining: (target?: number) => string }) {
  ...
  return <div className={`weekly-journey ${view.tone}`}>
    <section className="weekly-journey-card" aria-labelledby="weekly-journey-title">
      ...
      {completion?.completed && <div className="journey-results"><article><span aria-hidden="true">🏆</span><div><small>Melhor resultado</small><strong>{Number(completion.bestScore || 0).toLocaleString("pt-BR")} <em>pontos</em></strong></div></article><article><span aria-hidden="true">★</span><div><small>Sua colocação</small><strong>{data?.ranking?.position ? `${data.ranking.position}º` : "—"} <em>{data?.ranking?.provisional ? "Provisório" : ""}</em></strong></div></article></div>}
      ...
      <p className="journey-context">💡 {completion?.completed && attempts > 0 ? <>Você ainda possui <b>{attempts} tentativa(s)</b> para melhorar seu resultado nesta Jornada.</> : view.detail}</p>
