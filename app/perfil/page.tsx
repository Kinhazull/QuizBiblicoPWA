...
export default function Profile() {
  ...
  const { user, stats, progress, achievements, platformStatistics } = data;
  return <main className="profile-shell">
    ...
    <section className="profile-section" aria-labelledby="platform-progress-title"><h2 id="platform-progress-title">Progresso na plataforma</h2><div className="profile-stats"><article><small>Nível</small><strong>{progress?.level || 1}</strong></article><article><small>XP total</small><strong>{Number(progress?.totalXp || 0).toLocaleString("pt-BR")}</strong></article><article><small>Moedas</small><strong>{Number(progress?.coins || 0).toLocaleString("pt-BR")}</strong></article><article><small>Conquistas</small><strong>{Number(achievements?.unlocked || 0).toLocaleString("pt-BR")}</strong></article></div></section>
    <section className="profile-section" aria-labelledby="platform-statistics-title"><h2 id="platform-statistics-title">Atividade na plataforma</h2>{platformStatistics?.global?.lastActivityAt ? <div className="profile-stats"><article><small>Sessões concluídas</small><strong>{Number(platformStatistics.global.sessionsCompleted || 0).toLocaleString("pt-BR")}</strong></article><article><small>Jogos utilizados</small><strong>{Number(platformStatistics.global.gamesUsed || 0).toLocaleString("pt-BR")}</strong></article><article><small>Dias ativos</small><strong>{Number(platformStatistics.global.activeDays || 0).toLocaleString("pt-BR")}</strong></article><article><small>Sequência de atividade</small><strong>{Number(platformStatistics.global.currentDailyStreak || 0).toLocaleString("pt-BR")}</strong></article></div> : <p>Nenhuma atividade da plataforma registrada ainda.</p>}</section>
