...
export default function Profile() {
  ...
  return <main className="profile-shell">
    ...
    {["owner","admin","leader"].includes(user.role)&&<section className="profile-section mfa-profile-section" id="seguranca"><div><h2>Verificação em duas etapas</h2><p>{mfaStatus === "active" ? "A verificação em duas etapas está ativa nesta conta." : "Proteja o acesso administrativo com um aplicativo autenticador e códigos de recuperação MFA."}</p></div><a className="secondary" href="/configurar-mfa">{mfaStatus === "active" ? "Ver estado do MFA" : "Configurar MFA"}</a></section>}
