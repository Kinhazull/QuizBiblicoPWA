...
export default function Similar() {
  ...
  const question = (item: any) => <span><small>{item.reference} · {item.theme}</small><strong>{item.prompt}</strong><div className="similar-question-actions"><button onClick={() => setEditing(item.id)}>EDITAR</button><button className="danger" onClick={() => archive(item.id)}>EXCLUIR</button></div></span>;
