...
export default function RewardsPage(){
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    {data?<>
      ...
      <section aria-labelledby="achievements-title"><header className={styles.sectionHeader}><div><h2 id="achievements-title">Conquistas</h2><p>Feitos permanentes avaliados pelos serviços oficiais da plataforma.</p></div></header><div className={styles.achievementGrid}>{data.achievements.map(item=><article className={`${styles.achievement} ${item.state==="UNLOCKED"?styles.unlocked:styles.locked}`} key={item.code}><header><h3>{item.name}</h3><span className={styles.badge}>{stateLabel[item.state]}</span></header><p>{item.description}</p>{item.progress?<progress className={styles.progress} max={item.progress.target} value={item.progress.current} aria-label={`${item.name}: ${item.progress.current} de ${item.progress.target}`}/>:null}<footer><span>{item.rarity?rarity[item.rarity]:"Raridade não definida"}</span><span>{item.progress?`${item.progress.current}/${item.progress.target}`:"Progresso oculto"}</span><span>{item.reward?`+${item.reward.xp} XP · +${item.reward.coins} moedas`:"Sem recompensa"}</span>{item.unlockedAt?<time dateTime={new Date(item.unlockedAt).toISOString()}>{new Date(item.unlockedAt).toLocaleDateString("pt-BR")}</time>:null}</footer></article>)}</div></section>
    </>:null}
  </div></main>;
}
