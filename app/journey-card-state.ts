...
export function getJourneyCardView(data: JourneyCardData | null, remaining: (target?: number) => string): JourneyCardView {
  ...
  if (current) {
    ...
    if (attemptsLeft > 0) return { eyebrow: "JORNADA CONCLUÍDA", title: current.title, detail: `Melhor resultado: ${Number(completion.bestScore || 0).toLocaleString("pt-BR")} pontos · ${attemptsLeft} tentativa(s) opcional(is).`, action: "MELHORAR RESULTADO", href: "/jogar", tone: "completed", meta };
