...
export const onRequestGet=async({request,env}:{request:Request;env:AppEnv})=>{try{
  ...
  const stats:any=await env.DB.prepare(`SELECT COUNT(DISTINCT CASE WHEN status='completed' THEN round_id END) roundsPlayed,COUNT(CASE WHEN status='completed' THEN 1 END) attempts,COALESCE(MAX(score),0) bestScore,COALESCE(MAX(correct_answers),0) bestCorrect,COALESCE(MAX(max_streak),0) bestStreak,COALESCE(SUM(correct_answers),0) totalCorrect FROM attempts WHERE user_id=?1 AND mode='official'`).bind(user.id).first();
