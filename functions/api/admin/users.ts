...
export const onRequestPatch = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    const now = Date.now();
    const result = await env.DB.prepare(`UPDATE users SET status=COALESCE(?1,status), role=COALESCE(?2,role), approved_at=CASE WHEN ?1='active' THEN ?3 ELSE approved_at END, approved_by=CASE WHEN ?1='active' THEN ?4 ELSE approved_by END, updated_at=?3 WHERE id=?5 AND organization_id=?6 AND role NOT IN ('owner','admin')`).bind(status, role, now, admin.id, userId, admin.organizationId).run();
