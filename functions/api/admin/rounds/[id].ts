...
import { validateRoundConfiguration, validateStoredRound } from "../../../_lib/rounds";
...
export const onRequestPatch = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => {
  try {
    ...
    if (["scheduled","active"].includes(status)) { const validation=await validateStoredRound(env,params.id,admin.organizationId,{opensAt:Number(opensAt),closesAt:Number(closesAt),secondsPerQuestion:seconds,attemptLimit,seasonId}); if(!validation.ok)return json({error:"round_not_publishable",reason:validation.error},409); }
    await env.DB.batch([
