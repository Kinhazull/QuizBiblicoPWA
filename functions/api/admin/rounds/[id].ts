...
export const onRequestDelete = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => {
  try {
    ...
    if (Number(count?.total || 0) > 0) {
      await env.DB.prepare("UPDATE rounds SET status='cancelled',updated_at=?1 WHERE id=?2 AND organization_id=?3").bind(Date.now(), params.id, admin.organizationId).run();
      return json({ ok: true, archived: true });
    }
    await env.DB.prepare("DELETE FROM rounds WHERE id=?1 AND organization_id=?2").bind(params.id, admin.organizationId).run();
