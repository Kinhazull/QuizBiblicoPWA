...
export const onRequestGet = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    const [pending, members, rounds, review, stale, noSeason, expiringInvites, privacy, overlaps, activeSeason] = await Promise.all([
    ...
    add(!env.AI, { id: "ai", severity: "info", count: 1, title: "Sugestões com IA não configuradas", description: "O restante do aplicativo continua funcionando normalmente.", href: "/admin/perguntas/ia", action: "Ver orientação" });
    ...
    return json({ metrics: { pending, members, rounds, review, health: overlaps || noSeason || stale ? "attention" : "healthy" }, attention }, 200, { "Cache-Control": "no-store, private", "X-Content-Type-Options": "nosniff" });
