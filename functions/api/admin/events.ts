...
export const onRequestPost = async ({ request, env }: { request: Request; env: AppEnv }) => {
  ...
  catch (error) { return publicDomainError(error, { invalid_event: { category: PublicErrorCategory.VALIDATION_ERROR, status: 400 }, invalid_event_games: { category: PublicErrorCategory.VALIDATION_ERROR, status: 400 }, invalid_event_content: { category: PublicErrorCategory.VALIDATION_ERROR, status: 400 }, invalid_event_content_count: { category: PublicErrorCategory.VALIDATION_ERROR, status: 400 }, invalid_event_cover_asset: { category: PublicErrorCategory.VALIDATION_ERROR, status: 400 } }, { component: "admin-events", operation: "create_event" }); }
};
