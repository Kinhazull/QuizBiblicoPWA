...
export const onRequestGet = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    if (type === "members") {
      ...
      ({ results: rows } = await env.DB.prepare(`SELECT r.title AS round,u.display_name AS name,u.username,a.attempt_number AS attempt,a.mode,a.status,a.started_at AS startedAt,a.completed_at AS completedAt,COALESCE(a.resumed_count,0) AS resumedCount,a.last_resumed_at AS lastResumedAt,(SELECT COUNT(*) FROM attempt_answers aa WHERE aa.attempt_id=a.id) AS answered,r.closes_at AS closesAt FROM attempts a JOIN users u ON u.id=a.user_id JOIN rounds r ON r.id=a.round_id WHERE u.organization_id=?1 ORDER BY a.started_at DESC LIMIT 5000`).bind(admin.organizationId).all());
      const now = Date.now();
      rows = rows.map(row => ({ ...row,
        situation: row.status === "completed" ? "Concluída" : now > Number(row.closesAt) + graceMs ? "Abandonada/expirada" : "Em andamento",
        startedAt: new Date(row.startedAt).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" }),
        completedAt: row.completedAt ? new Date(row.completedAt).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" }) : "",
        lastResumedAt: row.lastResumedAt ? new Date(row.lastResumedAt).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" }) : "",
      }));
      columns = [{key:"round",label:"Rodada"},{key:"name",label:"Nome"},{key:"username",label:"Usuário"},{key:"attempt",label:"Tentativa"},{key:"mode",label:"Modo"},{key:"situation",label:"Situação"},{key:"answered",label:"Respostas salvas"},{key:"resumedCount",label:"Quantidade de retomadas"},{key:"startedAt",label:"Início"},{key:"lastResumedAt",label:"Última retomada"},{key:"completedAt",label:"Conclusão"}];
    } else if (type === "audit") {
