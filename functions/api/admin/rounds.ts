...
export const onRequestPost = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    const opensAt = new Date(body.opensAt).getTime(); const closesAt = new Date(body.closesAt).getTime();
    ...
    const roundId = crypto.randomUUID(); const now = Date.now(); const statements = [env.DB.prepare(`INSERT INTO rounds (id,organization_id,title,theme,description,status,opens_at,closes_at,official_attempt_limit,seconds_per_question,created_by,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,'scheduled',?6,?7,3,15,?8,?9,?9)`).bind(roundId, admin.organizationId, title, theme, body.description || null, opensAt, closesAt, admin.id, now)];
