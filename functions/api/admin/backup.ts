  ...
  try {
    ...
    const data = {
      ...
      tables: {
        ...
        content_review_comments: await rows(env, "SELECT * FROM content_review_comments WHERE organization_id=?1", organizationId),
        asset_registry: await rows(env, "SELECT * FROM asset_registry WHERE organization_id=?1", organizationId),
        content_assets: await rows(env, "SELECT * FROM content_assets WHERE organization_id=?1", organizationId),
