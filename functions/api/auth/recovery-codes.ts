...
export const onRequestPost=async({request,env}:{request:Request;env:AppEnv})=>{try{
  const user:any=await requireUser(request,env),fingerprint=await requestFingerprint(request),retry=await enforceRateLimit(env,`recovery-codes:${user.id}:${fingerprint}`,3,60*60*1000);if(retry)return json({error:'too_many_requests',retryAfter:retry},429,{'retry-after':String(retry)});const body:any=await request.json();
  ...
  await env.DB.batch(statements);return json({codes,warning:'Guarde estes cÃ³digos agora. Eles nÃ£o serÃ£o exibidos novamente.'});
