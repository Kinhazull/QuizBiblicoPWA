...
export const onRequestGet = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    if (type === "weekly") {
      ...
      query = `SELECT u.id,u.display_name AS displayName,MAX(a.score) AS score,MAX(a.correct_answers) AS correctAnswers,MIN(a.total_time_ms) AS totalTimeMs FROM users u JOIN attempts a ON a.user_id=u.id WHERE u.organization_id=?1 AND a.round_id=?2 AND a.mode='official' AND a.status='completed' GROUP BY u.id ORDER BY score DESC,correctAnswers DESC,totalTimeMs ASC LIMIT 100`;
    } else if (type === "average") {
      query = `WITH best AS (SELECT user_id,round_id,MAX(score) score FROM attempts WHERE mode='official' AND status='completed' GROUP BY user_id,round_id) SELECT u.id,u.display_name AS displayName,ROUND(AVG(best.score)) AS score,COUNT(*) AS roundsPlayed FROM users u JOIN best ON best.user_id=u.id WHERE u.organization_id=?1 GROUP BY u.id HAVING COUNT(*)>=3 ORDER BY score DESC LIMIT 100`;
    } else {
      query = `WITH best AS (SELECT user_id,round_id,MAX(score) score FROM attempts WHERE mode='official' AND status='completed' GROUP BY user_id,round_id) SELECT u.id,u.display_name AS displayName,SUM(best.score) AS score,COUNT(*) AS roundsPlayed FROM users u JOIN best ON best.user_id=u.id WHERE u.organization_id=?1 GROUP BY u.id ORDER BY score DESC LIMIT 100`;
    }
