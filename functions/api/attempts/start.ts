...
export const onRequestPost=async({request,env}:{request:Request;env:AppEnv})=>{try{
  ...
  if(existing){
    ...
    const questions=await hydrate(env,roundId,existing.shuffle_seed,existing.question_order_json);if(questions.length!==QUESTION_COUNT||questions.some(question=>question.choices.length!==4))return json({error:"invalid_round_content"},409);const progress:any=await env.DB.prepare("SELECT COUNT(*) answered,COALESCE(SUM(points),0) score FROM attempt_answers WHERE attempt_id=?1").bind(existing.id).first(),nextIndex=Number(progress?.answered||0);let questionStartedAt=Number(existing.current_question_started_at||0);if(!questionStartedAt&&nextIndex<questions.length){questionStartedAt=now;await env.DB.prepare("UPDATE attempts SET current_question_started_at=?1 WHERE id=?2 AND current_question_started_at IS NULL").bind(now,existing.id).run()}const remaining=Math.max(0,Number(round.seconds_per_question)-Math.floor((now-questionStartedAt)/1000));
    return json({attempt:{id:existing.id,attemptNumber:existing.attempt_number,mode,secondsPerQuestion:round.seconds_per_question,questions,resumed:true,nextIndex,score:Number(progress?.score||0),remainingSeconds:remaining,questionStartedAt,graceDeadline:attemptGraceDeadline(Number(round.closes_at))}});
  }
  ...
  try{await env.DB.prepare("INSERT INTO attempts (id,user_id,round_id,attempt_number,mode,status,shuffle_seed,question_order_json,started_at,current_question_started_at) VALUES (?1,?2,?3,?4,?5,'in_progress',?6,?7,?8,?8)").bind(id,user.id,roundId,attemptNumber,mode,seed,questionOrder,now).run()}catch{const winner:any=await env.DB.prepare("SELECT * FROM attempts WHERE user_id=?1 AND round_id=?2 AND mode=?3 AND status='in_progress' ORDER BY started_at DESC LIMIT 1").bind(user.id,roundId,mode).first();if(winner){const validQuestions=await hydrate(env,roundId,winner.shuffle_seed,winner.question_order_json),progress:any=await env.DB.prepare("SELECT COUNT(*) answered,COALESCE(SUM(points),0) score FROM attempt_answers WHERE attempt_id=?1").bind(winner.id).first();let base=Number(winner.current_question_started_at||0);if(!base){base=Date.now();await env.DB.prepare("UPDATE attempts SET current_question_started_at=?1 WHERE id=?2 AND current_question_started_at IS NULL").bind(base,winner.id).run()}const remaining=Math.max(0,Number(round.seconds_per_question)-Math.floor((Date.now()-base)/1000));return json({attempt:{id:winner.id,attemptNumber:winner.attempt_number,mode,secondsPerQuestion:round.seconds_per_question,questions:validQuestions,resumed:true,nextIndex:Number(progress?.answered||0),score:Number(progress?.score||0),remainingSeconds:remaining,questionStartedAt:base,graceDeadline:attemptGraceDeadline(Number(round.closes_at))}})}return json({error:"attempt_conflict"},409)}
