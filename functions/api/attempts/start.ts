...
async function hydrate(env:AppEnv,roundId:string,seed:string,storedOrder?:string|null){
  const [questionRows,choiceRows]=await Promise.all([
    ...
    env.DB.prepare("SELECT c.id,c.text,c.question_id questionId FROM choices c JOIN questions q ON q.id=c.question_id WHERE q.round_id=?1 AND q.active=1").bind(roundId).all()
  ]);
...
export const onRequestPost=async({request,env}:{request:Request;env:AppEnv})=>{try{
  const user:any=await requireUser(request,env),body:any=await request.json(),roundId=String(body.roundId||""),mode=body.mode==="practice"?"practice":"official",now=Date.now();
  ...
  if(mode==="practice"){
    ...
    const official:any=await env.DB.prepare("SELECT COUNT(*) total FROM attempts WHERE user_id=?1 AND round_id=?2 AND mode='official' AND status='completed'").bind(user.id,roundId).first();
  ...
  if(existing){
    ...
    const questions=await hydrate(env,roundId,existing.shuffle_seed,existing.question_order_json),progress:any=await env.DB.prepare("SELECT COUNT(*) answered,COALESCE(SUM(points),0) score,MAX(answered_at) lastAnsweredAt FROM attempt_answers WHERE attempt_id=?1").bind(existing.id).first(),nextIndex=Number(progress?.answered||0),base=Number(progress?.lastAnsweredAt||existing.started_at),remaining=Math.max(0,Number(round.seconds_per_question)-Math.floor((now-base)/1000));
  ...
  const count:any=await env.DB.prepare("SELECT COUNT(*) total FROM attempts WHERE user_id=?1 AND round_id=?2 AND mode=?3").bind(user.id,roundId,mode).first(),attemptNumber=Number(count?.total||0)+1;
  if(mode==="official"&&attemptNumber>Number(round.official_attempt_limit))return json({error:"attempt_limit"},403);
  const seed=randomToken(18),questions=await hydrate(env,roundId,seed),id=crypto.randomUUID(),questionOrder=JSON.stringify(questions.map(question=>question.id));
  try{await env.DB.prepare("INSERT INTO attempts (id,user_id,round_id,attempt_number,mode,status,shuffle_seed,question_order_json,started_at) VALUES (?1,?2,?3,?4,?5,'in_progress',?6,?7,?8)").bind(id,user.id,roundId,attemptNumber,mode,seed,questionOrder,now).run()}catch{return json({error:"attempt_already_started"},409)}
