...
export const onRequestPost = async ({ request, env }: { request: Request; env: AppEnv }) => {
  try {
    ...
    if(existing){const questionsResult=await env.DB.prepare(`SELECT id,reference,prompt,commentary FROM questions WHERE round_id=?1 AND active=1`).bind(roundId).all(),questions=seededShuffle(questionsResult.results as any[],existing.shuffle_seed),hydrated=[];for(const question of questions){const choices=await env.DB.prepare(`SELECT id,text FROM choices WHERE question_id=?1`).bind((question as any).id).all();hydrated.push({id:(question as any).id,reference:(question as any).reference,prompt:(question as any).prompt,choices:seededShuffle(choices.results as any[],`${existing.shuffle_seed}:${(question as any).id}`)})}const progress:any=await env.DB.prepare(`SELECT COUNT(*) answered,COALESCE(SUM(points),0) score,MAX(answered_at) lastAnsweredAt FROM attempt_answers WHERE attempt_id=?1`).bind(existing.id).first(),nextIndex=Number(progress?.answered||0),base=Number(progress?.lastAnsweredAt||existing.started_at),remaining=Math.max(0,Number(round.seconds_per_question)-Math.floor((now-base)/1000));return json({attempt:{id:existing.id,attemptNumber:existing.attempt_number,mode,secondsPerQuestion:round.seconds_per_question,questions:hydrated,resumed:true,nextIndex,score:Number(progress?.score||0),remainingSeconds:remaining}},200)}
