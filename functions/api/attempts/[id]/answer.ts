...
export const onRequestPost = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => {
  try {
    ...
    const now = Date.now();
    const attempt: any = await env.DB.prepare("SELECT a.*,r.seconds_per_question,r.closes_at,r.status round_status FROM attempts a JOIN rounds r ON r.id=a.round_id WHERE a.id=?1 AND a.user_id=?2 AND a.status='in_progress'").bind(params.id, user.id).first();
    ...
    const points = correct ? Math.max(100, 400 + Math.floor((attempt.seconds_per_question * 1000 - elapsed) / 1000) * 40 + streak * 100) : 0;
