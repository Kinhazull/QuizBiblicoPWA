...
export const onRequestPost = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => {
  try {
    ...
    const body: any = await request.json(), questionId = String(body?.questionId || ""), choiceId = String(body?.choiceId || "");
    ...
    const correct = !serverTimedOut && Boolean(choice.correct), rules = scoringRules(parsedRules), points = calculateAnswerPoints({ correct, elapsedMs: elapsed, secondsPerQuestion: Number(attempt.seconds_per_question), currentStreak, rules });
