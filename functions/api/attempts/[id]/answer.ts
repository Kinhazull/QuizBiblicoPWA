...
export const onRequestPost = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => {
  try {
    ...
    const attempt: any = await env.DB.prepare(`SELECT a.*,r.seconds_per_question,r.closes_at,r.status round_status FROM attempts a JOIN rounds r ON r.id=a.round_id WHERE a.id=?1 AND a.user_id=?2 AND a.status='in_progress'`).bind(params.id, user.id).first(); if (!attempt || now>=Number(attempt.closes_at) || !['scheduled','active'].includes(attempt.round_status)) return json({ error: "attempt_unavailable" }, 404);
    const questionId = String(body.questionId || ""); const choiceId = String(body.choiceId || "");
    ...
    const previous: any = await env.DB.prepare(`SELECT correct,answered_at FROM attempt_answers WHERE attempt_id=?1 ORDER BY question_order DESC`).bind(params.id).all(); const order=previous.results.length;const base=order?Number((previous.results[0] as any).answered_at):Number(attempt.started_at);const elapsed=Math.max(500,Math.min(now-base,attempt.seconds_per_question*1000));let streak = 0; for (const answer of previous.results as any[]) { if (!answer.correct) break; streak++; }
