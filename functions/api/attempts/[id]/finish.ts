...
export const onRequestPost = async ({ request, env, params }: { request: Request; env: AppEnv; params: { id: string } }) => { try {
  ...
  const updated = await env.DB.prepare("UPDATE attempts SET status='completed',score=?1,correct_answers=?2,total_time_ms=?3,max_streak=?4,completed_at=?5 WHERE id=?6 AND status='in_progress'").bind(summary.score, summary.correctAnswers, summary.totalTimeMs, summary.maxStreak, now, params.id).run();
  if (!updated.meta.changes) { const winner: any = await env.DB.prepare("SELECT * FROM attempts WHERE id=?1 AND user_id=?2 AND status='completed'").bind(params.id, user.id).first(); return winner ? completed(winner, true) : json({ error: "attempt_conflict" }, 409); }
  await env.DB.prepare("INSERT INTO audit_logs(id,organization_id,actor_user_id,action,entity_type,entity_id,details_json,created_at) VALUES(?1,?2,?3,'attempt.completed','attempt',?4,?5,?6)").bind(crypto.randomUUID(), user.organizationId, user.id, params.id, JSON.stringify({ mode: attempt.mode, roundId: attempt.round_id, score: summary.score }), now).run();
  await syncBadges(env, user.id); return json({ ok: true, alreadyFinished: false, result: summary });
