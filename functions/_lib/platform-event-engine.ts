...
async function claimConsumer(env: AppEnv, eventId: string, consumer: CoreEventConsumer, now: number) {
  ...
  let row = await env.DB.prepare("SELECT state,lease_token leaseToken,lease_until leaseUntil FROM core_platform_event_processing WHERE event_id=?1 AND consumer_id=?2 AND handler_version=?3").bind(eventId, consumer.id, consumer.handlerVersion).first<any>();
  ...
  const recovered = await env.DB.prepare(`UPDATE core_platform_event_processing SET state='processing',attempt_count=attempt_count+1,lease_token=?1,lease_until=?2,last_error_code=NULL,updated_at=?3
    WHERE event_id=?4 AND consumer_id=?5 AND handler_version=?6 AND (state='retryable_failed' OR (state='processing' AND lease_until<=?3))`).bind(leaseToken, now + LEASE_MS, now, eventId, consumer.id, consumer.handlerVersion).run();
...
export async function publishCoreEvent(env: AppEnv, event: CorePlatformEvent, consumers: readonly CoreEventConsumer[] = [], now = Date.now()) {
  ...
  for (const consumer of selected) {
    ...
    try {
      ...
      await env.DB.prepare(`UPDATE core_platform_event_processing SET state='completed',processed_at=?1,lease_token=NULL,lease_until=NULL,last_error_code=NULL,updated_at=?1
        WHERE event_id=?2 AND consumer_id=?3 AND handler_version=?4 AND lease_token=?5`).bind(Date.now(), event.eventId, consumer.id, consumer.handlerVersion, claim.leaseToken).run();
      results.push({ consumerId: consumer.id, status: "completed" });
    } catch (error) {
      await env.DB.prepare(`UPDATE core_platform_event_processing SET state='retryable_failed',lease_token=NULL,lease_until=NULL,last_error_code=?1,updated_at=?2
        WHERE event_id=?3 AND consumer_id=?4 AND handler_version=?5 AND lease_token=?6`).bind(safeErrorCode(error), Date.now(), event.eventId, consumer.id, consumer.handlerVersion, claim.leaseToken).run();
      results.push({ consumerId: consumer.id, status: "retryable_failed" });
    }
  }
  ...
  await env.DB.prepare("UPDATE core_platform_events SET status=?1,updated_at=?2 WHERE event_id=?3").bind(status, Date.now(), event.eventId).run();
